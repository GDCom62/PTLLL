...
def main():
    ...
    # Inicializa a memória de segurança do Streamlit para evitar travar as listas
            ...
            if len(equipamentos) > 0 and isinstance(equipamentos, list):
                for eq in equipamentos:
                    ...
                    if st.button("🗑️ Remover " + str(eq['id']), key="del_" + str(eq['id'])):
                        if not MODO_DEMO:
                            try:
                                requests.delete(SUB_URL + "/rest/v1/equipamentos?id=eq." + str(eq['id']), headers=SUB_HEADERS, timeout=5)
                            except:
                                pass
            ...
            with st.form("form_cadastro"):
                ...
                if st.form_submit_button("Salvar Equipamento"):
                    if id_eq and nome_eq:
                        ...
                        if not MODO_DEMO:
                            try:
                                requests.post(SUB_URL + "/rest/v1/equipamentos", json=novo_registro, headers=SUB_HEADERS, timeout=5)
                            except:
                                pass
            ...
            if opcoes_edicao:
                ...
                with st.form("form_edicao"):
                    ...
                    if st.form_submit_button("Gravar Alterações"):
                        ...
                        if not MODO_DEMO:
                            try:
                                requests.patch(SUB_URL + "/rest/v1/equipamentos?id=eq." + str(eq_para_editar['id']), json=alteracoes, headers=SUB_HEADERS, timeout=5)
                            except:
                                pass
