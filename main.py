...
# --- MEMÓRIA LOCAL DE SEGURANÇA (PREENCHIDA DE FÁBRICA PARA TESTE IMEDIATO) ---
...
# OS #1 já inicia aberta por padrão para você poder emitir a PT no mesmo segundo
...
# --- INJEÇÃO DE MÁQUINAS NA NUVEM SE ESTIVER VAZIO ---
if not MODO_DEMO:
    try:
...
# ==========================================
# PAGE 4: PLANEJAMENTO TEMPORAL (COMPLETO)
# ==========================================
elif menu == "📅 Planejamento & Checklists":
    st.header("📅 Planejamento de Manutenções Preventivas")
    ...
    with st.form("form_novo_planejamento_topo"):
        ...
        if st.form_submit_button("💾 Gravar e Agendar Manutenção"):
            if eq_escolhido != "Nenhum equipamento cadastrado":
                novo_agendamento = {"id": len(st.session_state.planejamento_local) + 1, "equipamento": eq_escolhido, "periodo": periodo_escolhido, "data_prevista": data_planejada.strftime("%d/%m/%Y"), "pecas": pecas_necessarias, "status": "Pendente", "seguranca": "Uso de EPIs obrigatório."}
                st.session_state.planejamento_local.append(novo_agendamento)
                if not MODO_DEMO:
                    try: requests.post(SUB_URL + "/rest/v1/planejamento", json=novo_agendamento, headers=SUB_HEADERS, timeout=5)
                    except: pass
                st.success("Manutenção gravada na agenda com sucesso!")
                st.rerun()
